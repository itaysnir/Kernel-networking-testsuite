...
run_test() {
	...
	sudo netperf -H $dip1 -p $(( REMOTE_PORT + 1)) -t TCP_STREAM -T 0,2 -l $TIMEOUT -- -m$CHUNK_SIZE -s16M &
	sudo netperf -H $dip1 -p $(( REMOTE_PORT + 2)) -t TCP_STREAM -T 0,3 -l $TIMEOUT -- -m$CHUNK_SIZE -s16M &
	sudo netperf -H $dip1 -p $(( REMOTE_PORT + 3)) -t TCP_STREAM -T 0,4 -l $TIMEOUT -- -m$CHUNK_SIZE -s16M &
	sudo netperf -H $dip1 -p $(( REMOTE_PORT + 4)) -t TCP_STREAM -T 0,5 -l $TIMEOUT -- -m$CHUNK_SIZE -s16M &
...
main() {
	init_env
	run_test_multiple_times "netserver" 5
	generate_plots	
}
